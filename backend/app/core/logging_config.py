...
def setup_logging(
    level: str = "INFO", log_dir: Optional[Path] = None, enable_json: bool = True, enable_file_rotation: bool = True
) -> None:
    """
    ...
    # Setup formatters
    if enable_json:
        json_formatter = ContextualJsonFormatter(fmt="%(timestamp)s %(level)s %(name)s %(message)s")
    ...
    if log_dir:
        ...
        if enable_file_rotation:
            ...
            main_handler = RotatingFileHandler(main_log_file, maxBytes=10 * 1024 * 1024, backupCount=10)  # 10MB
        else:
        ...
        if enable_file_rotation:
            error_handler = RotatingFileHandler(error_log_file, maxBytes=5 * 1024 * 1024, backupCount=5)  # 5MB
        else:
        ...
        daily_handler = TimedRotatingFileHandler(
            daily_log_file, when="midnight", interval=1, backupCount=30  # Keep 30 days
        )
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.debug(f"Function {func.__name__} completed successfully", extra={"function": func.__name__})
                return result
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            ...
            if exc_type is None:
                logger.info(
                    ...
                    extra={"operation": operation, "duration_seconds": round(duration, 3), "status": "success"},
                )
            else:
                logger.error(
                    f"Failed {operation}",
