...
async def download_model(job_id: str):
    ...
    if status != "completed":
        raise HTTPException(
            status_code=400, detail=f"Job status is '{status}', model download is only available for completed jobs"
        )

    if not model_path:
        raise HTTPException(status_code=500, detail="Model path not found in job metadata")
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=model_{job_id}.zip"},
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to create model archive: {str(e)}")
...
async def export_model(job_id: str, request: ExportModelRequest):
    ...
    if status != "completed":
        raise HTTPException(
            status_code=400, detail=f"Job status is '{status}', model export is only available for completed jobs"
        )
    ...
    try:
        ...
        if not export_path:
            raise HTTPException(status_code=500, detail=f"Failed to export model to {request.format} format")
