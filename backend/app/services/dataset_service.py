...
class DatasetService:
        ...
        try:
            async with aiofiles.open(file_path, encoding="utf-8") as f:
                ...
                if isinstance(data, list):
                    return {"type": "json_array", "num_rows": len(data), "sample": data[:3] if len(data) > 0 else []}
    ...
    async def _preview_jsonl(self, file_path: Path) -> dict:
        ...
        try:
            lines = []
            async with aiofiles.open(file_path, encoding="utf-8") as f:
                for _i in range(5):  # Read first 5 lines
                    line = await f.readline()
                    if not line:
                        break
                    lines.append(json.loads(line.strip()))

            return {"type": "jsonl", "num_rows": len(lines), "sample": lines[:3]}
        except Exception as e:
    ...
    async def _preview_csv(self, file_path: Path) -> dict:
        ...
        try:
            lines = []
            async with aiofiles.open(file_path, encoding="utf-8") as f:
                for _i in range(6):  # Header + 5 rows
                    line = await f.readline()
                    if not line:
                        break
            ...
            if lines:
                ...
                return {"type": "csv", "num_columns": len(header), "column_names": header, "sample_rows": rows[:3]}
        except Exception as e:
    ...
    async def _preview_text(self, file_path: Path) -> dict:
        ...
        try:
            async with aiofiles.open(file_path, encoding="utf-8") as f:
                ...
                return {"type": "text", "preview": content}
        except Exception as e:
